...
class DefaultUserSettings
{
    ...
    public function __construct(\ilSetting $settings)
    {
        ...
        $this->default_user_role_id = (int) $settings->get(\EventoImport\config\CronConfigForm::CONF_DEFAULT_USER_ROLE); // TODO: Use default user role from constants
        ...
        $import_acc_duration_in_months = (int) $settings->get(\EventoImport\config\CronConfigForm::CONF_USER_IMPORT_ACC_DURATION, 12);
        $this->acc_duration_after_import = $this->addMonthsToCurrent($import_acc_duration_in_months);
        $max_acc_duration_in_months = (int) $settings->get(\EventoImport\config\CronConfigForm::CONF_USER_MAX_ACC_DURATION, 24);
        $this->max_acc_duration = $this->addMonthsToCurrent($max_acc_duration_in_months);

        $this->auth_mode = $settings->get(\EventoImport\config\CronConfigForm::CONF_USER_AUTH_MODE, 'local');
        ...
        $role_mapping = $settings->get(\EventoImport\config\CronConfigForm::CONF_ROLES_ILIAS_EVENTO_MAPPING);
