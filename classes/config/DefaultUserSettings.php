...
class DefaultUserSettings
{
    ...
    public function __construct(\ilSetting $settings)
    {
        ...
        $import_acc_duration_in_months = (int) $settings->get(\ilEventoImportCronConfig::CONF_USER_IMPORT_ACC_DURATION, 12);
        $this->acc_duration_after_import = $this->addMonthsToCurrent($import_acc_duration_in_months);
        $max_acc_duration_in_months = (int) $settings->get(\ilEventoImportCronConfig::CONF_USER_MAX_ACC_DURATION, 24);
        $this->max_acc_duration = $this->addMonthsToCurrent($max_acc_duration_in_months);

        $this->auth_mode = $settings->get(\ilEventoImportCronConfig::CONF_USER_AUTH_MODE, 'local');
