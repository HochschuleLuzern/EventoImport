...
class ilEventoImportImportMemberships
{
    ...
    private function assignToRole($user_id, $role_id)
    {
        ...
        if (!($assigned = $this->rbacreview->isAssigned($user_id, $role_id)) && $this->rbacadmin->assignUser($role_id, $user_id)) {
            ...
            $r = $this->ilDB->queryF("SELECT 1 FROM crnhk_crevento_subs WHERE evento_event_id=%s AND evento_user_id=%s LIMIT 1", array("integer", "integer"), array($role_id, $user_id));
            if ($this->ilDB->numRows($r) != 1) {
                $this->evento_logger->log(ilEventoImportLogger::CREVENTO_SUB_NEWLY_ADDED, array("usr_id" => $user_id, "role_id" => $role_id));
            }
        }
    }
    ...
    private function getDeletedUsersInRole($role_id, $user_ids)
    {
        ...
        $r = $this->ilDB->queryF("SELECT evento_user_id FROM crnhk_crevento_subs WHERE evento_event_id=%s AND update_info_code!=%s", array("integer", "integer"), array($role_id, ilEventoImportLogger::CREVENTO_SUB_REMOVED));
    ...
    {
        $r = $this->ilDB->queryF("SELECT * FROM crnhk_crevento_subs WHERE evento_user_id=%s AND evento_event_id=%s AND update_info_code IN (%s, %s, %s)", array('integer', 'integer', 'integer', 'integer', 'integer'), array($user_id, $role_id, ilEventoImportLogger::CREVENTO_SUB_NEWLY_ADDED, ilEventoImportLogger::CREVENTO_SUB_CREATED, ilEventoImportLogger::CREVENTO_SUB_UPDATED));
