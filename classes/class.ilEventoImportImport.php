...
class ilEventoImportImport extends ilCronJob
{
    ...
    private function addNewEventoImprotSettingsToForm(ilPropertyFormGUI $form)
    {
        /***************************
         * Location Settings
         ***************************/
        $header = new ilFormSectionHeaderGUI();
        $header->setTitle($this->cp->txt(''));
        $form->addItem($header);

        $json_settings = $this->settings->get('');
        $locations_settings = json_decode($json_settings, true);

        $departments = new ilTextInputGUI($this->cp->txt(''), '');
        $departments->setMulti(true, false, true);
        if (isset($locations_settings['departments']) && is_array($locations_settings['departments'])) {
            $departments->setValue($locations_settings['']);
        }
        $form->addItem($departments);

        $kinds = new ilTextInputGUI($this->cp->txt(''), '');
        $kinds->setMulti(true, false, true);
        if (isset($locations_settings['kinds']) && is_array($locations_settings[''])) {
            $kinds->setValue($locations_settings['kinds']);
        }
        $form->addItem($kinds);

        $years = new ilTextInputGUI($this->cp->txt(''), '');
        $years->setMulti(true, false, true);
        if (isset($locations_settings['']) && is_array($locations_settings['years'])) {
        ...
        $ws_item->setInfo($this->cp->txt(''));
        $ws_item->setRequired(true);
        $ws_item->setValue($this->settings->get('', ''));
        ...
        $ws_item->setValue($this->settings->get('', '0'));
        $form->addItem($ws_item);
        ...
        $ws_item = new ilNumberInputGUI(
            $this->cp->txt('api_timeout'),
            'crevento_api_timeout'
        );
        $ws_item->setInfo($this->cp->txt('api_timeout_desc'));
        $ws_item->setRequired(true);
        $ws_item->allowDecimals(false);
        $ws_item->setValue($this->settings->get('crevento_api_timeout', '0'));
        $form->addItem($ws_item);
        ...
        $ws_item->setRequired(true);
        $ws_item->allowDecimals(false);
        $ws_item->setValue($this->settings->get('crevento_api_max_retries', '0'));
        $form->addItem($ws_item);
        ...
        $ws_item = new ilNumberInputGUI(
            $this->cp->txt(''),
            'crevento_user_acc_validity_time'
        );
        $ws_item->setInfo($this->cp->txt(''));
        $ws_item->setRequired(true);
        $ws_item->allowDecimals(false);
        $ws_item->setValue($this->settings->get('crevento_user_acc_validity_time'));
        $form->addItem($ws_item);
        ...
        $ws_item->setRequired(true);
        $ws_item->allowDecimals(false);
        $ws_item->setValue($this->settings->get('crevento_default_user_role'));
        $form->addItem($ws_item);

        $section = new ilFormSectionHeaderGUI();
        $section->setTitle($this->cp->txt(''));
        ...
        foreach ($global_roles as $role_id) {
            ...
            $ws_item->addSubItem(new ilNumberInputGUI($this->cp->txt(''), "map_from_$role_id"));
            $form->addItem($ws_item);
        }
        ...
        $radio = new ilRadioGroupInputGUI($this->cp->txt(''), 'crevento_object_owner');
        $option = new ilRadioOption($this->cp->txt(''), 'root');
        $radio->addOption($option);

        $option = new ilRadioOption($this->cp->txt(''), 'first_admin');
        $radio->addOption($option);

        $option = new ilRadioOption($this->cp->txt(''), 'custom_user');
        $ws_item = new ilNumberInputGUI(
            $this->cp->txt(''),
            'crevento_object_owner_id'
        );
        $ws_item->allowDecimals(false);
        $option->addSubItem($ws_item);
...
//        $this->addNewEventoImprotSettingsToForm($a_form);
        ...
        foreach ($auth_modes as $auth_mode => $auth_name) {
            ...
            } elseif ($this->settings->get($auth_name . '_active') || $auth_mode == AUTH_LOCAL) {
                $options[$auth_name] = $auth_name;
            }
        }
    ...
    {
        $conf = new ilEventoImportCronConfig($this->settings, $this->cp);
        $conf->saveCustomCronJobSettings($a_form);
        return;
